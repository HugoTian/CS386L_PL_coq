...
Proof.
    intros X P H. intros [x Px]. unfold not in Px. apply Px. apply H. Qed.
(** [] *)
...
Proof.
  intros X Q P. split. intros [x  [Qx |Px ] ]. left. exists x. apply Qx. right. exists x. apply Px.
  intros [[x Qx] | [ x Px]]. exists x. left. apply Qx.  exists x. right. apply Px.
Qed.   
(** [] *)
...
Proof.
   intros A B f l y.
   split. induction l as [ | n l'].
          - intros H.  inversion H.
          - intros [H1 | H2]. exists n. split. apply H1. simpl. left. reflexivity.
            apply IHl' in H2. inversion H2. exists x. split. apply H. right. apply H.
...
Proof.
    ...
    - induction l as [ | x ll].
      ...
      +intros [H1 | H2]. left. rewrite <- H1. simpl. left. reflexivity.
       apply IHll in H2. simpl. rewrite <- or_assoc. right. apply H2.
    - induction l as [ | t ll]. 
      + simpl. intros [H1  | H3] . inversion H1. apply H3.
      + simpl. intros [[H1 | H2] | H3]. left. apply H1. right. apply IHll. left. apply H2. right. apply IHll. right. apply H3.
Qed.
...
Proof.
   ...
   - induction l as [ | n ll].
      +simpl. reflexivity.
      + intros H. split.  apply H. simpl. left. reflexivity. apply IHll. intros x H1. apply H. right. apply H1.
   - induction l as [ | n ll].
      +simpl. intros t f p. inversion p.
      + simpl. intros H1 H2. intros  [H3 | H4]. rewrite <- H3. apply H1. apply IHll. apply H1. apply H4.
Qed.
(** [] *)
...
Proof.
    intros Podd Peven n.
    intros H1 H2. unfold combine_odd_even in H1. rewrite H2 in H1.
    apply H1.
Qed.
...
Proof. intros X. apply functional_extensionality.  intros l. induction l as [ | n l'].
   - simpl. unfold tr_rev. simpl. reflexivity.
   - simpl. unfold tr_rev. simpl. unfold tr_rev in IHl'. rewrite <- IHl'.
   rewrite rev_append_one. reflexivity.
Qed.
...
Proof.
  ...
  - rewrite->evenb_S. destruct evenb. 
     ...
     + simpl. inversion IHn'. exists (S x). rewrite H. simpl. reflexivity.
Qed.
...
Proof.
  intros b1 b2. split.
  - destruct b1. 
    + destruct b2.  intros H. split. reflexivity. reflexivity. intros H. inversion H.
    + destruct b2. intros H. inversion H. intros H. inversion H.
  - destruct b1.
    + destruct b2. intros H.  reflexivity. intros [H1 H2]. inversion H2.
    + destruct b2. intros H. inversion H. inversion H0. intros H. inversion H. inversion H0.
Qed.
...
Proof.
   ...
   -  destruct b1.
    ...
    + intros [H1 | H2]. inversion H1. apply H2.
Qed. 
(** [] *)
...
Fixpoint beq_list {A} (beq : A -> A -> bool)
  ...
  match l1 , l2 with
  | [], [] => true 
  | x1 :: x, y1 :: y => if beq x1 y1 then beq_list beq x y else false
  | _, _ => false
  end.
...
Proof.
   intros A beq H2.
   induction l1 as [ | x l].
   - destruct l2 as [ | x1 l1].
      + split. simpl. reflexivity. reflexivity.
      + simpl. split. intros contra. inversion contra. intros contra. inversion contra.
   - destruct l2 as [ | x2 l2].
      + split. simpl. intros contra. inversion contra. intros contra. inversion contra.
      + split. intros H. apply andb_true_iff in H. destruct H as [H1 H3]. apply H2 in H1. apply IHl in H3.
         rewrite -> H1. rewrite -> H3 . reflexivity.
        intros H.  apply andb_true_iff. split. apply H2 . inversion H. reflexivity.
...
Theorem excluded_middle_irrefutable:  forall (P:Prop),
  ~ ~ (P \/ ~ P).
Proof.
intros P. intros H. unfold not in H. apply H. right. intros. apply H.
