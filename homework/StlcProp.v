...
(* FILL IN HERE *)
  ...
  (* I think it is not hold,
      tif ttrue tfalse (tvar x)
     break the property.
   *)
...
Theorem progress_exercise : forall t T, empty |- t \in T -> value t \/ exists t', t ==> t'.

Theorem preservation_exercise : forall t t' T, empty |- t \in T -> t ==> t' -> empty |- t' \in T.
...
and the following typing rule:
    ...
    false, give a counterexample.
      ...
      - Determinism of [step]
          becomes false ,  tif ttrue ttrue ttrue ==> ttrue/zap, non deterministic result
      - Progress
          remains true
      - Preservation
          remains true
...
(** Suppose instead that we add a new term [foo] with the following 
    ...
    false, give a counterexample.
      ...
      - Progress
          remains true
      - Preservation
          becomes false,  foo => true where foo has type bool -> bool
...
(** Suppose instead that we remove the rule [ST_App1] from the [step]
    ...
    false, give a counterexample.
      ...
      - Determinism of [step]
          remains true
      - Progress
          becomes false, not yet think of a counter example yet
      - Preservation
          remains true
[]
*)
...
Fixpoint subst (x : id) (s : tm) (t : tm) : tm :=
  match t with
    | tvar i   => if beq_id i x then s else tvar i
    | tapp a b => tapp ([x := s] a) ([x := s] b)
    | tabs i tp bd => if beq_id i x then tabs i tp bd
...
Reserved Notation "lambda '|-' t '\in' T" (at level 40).
...
Inductive has_type : context -> tm -> ty -> Prop :=
  ...
  | T_If0 : forall G c t e T,
      ...
      G |- e \in T ->
      G |- (tif0 c t e) \in T
  where " lambda '|-' t '\in' T" := (has_type lambda t T).
...
Lemma canonical_forms_nat : forall t,
  ...
  (exists n, t = tnat n).
Proof.
  intros. inversion H; subst; inversion H0; eauto.
Qed.
...
Lemma free_in_context : forall x t T lambda,
  appears_free_in x t ->
  lambda |- t \in T ->
  exists T', lambda x = Some T'.
Proof.
  intros x t.
  induction t; intros; inversion H0; subst; inversion H; subst; eauto.
  - apply IHt in H6; auto.
      destruct H6. unfold pupdate in H1. rewrite t_update_neq in H1; eauto.
Qed.
