...
Proof.
  ...
  - (* n = 0 *)    reflexivity.
  - (* n = S n' *) simpl.  rewrite <- In'. reflexivity.  Qed.
...
(* FILL IN HERE *)
  Basically, destruct is proof by cases, and there is no induction invloved.
  However, induction has the inductive step, and apply the hypothesis.
  Moreover, I think the induction should work on type with successor (
  or something similar to successor). However, destruct can work without
  successor
*)
(** [] *)
...
Proof.
  ...
  rewrite -> plus_assoc.
  rewrite -> plus_assoc.
  assert (H : n + m = m + n). { rewrite -> plus_comm. 
  reflexivity. }
  rewrite -> H.
  reflexivity.
 Qed.
...
Proof.
  ...
  -simpl. rewrite <- mult_n_O. rewrite <- mult_n_O. rewrite <- mult_n_O. reflexivity.
...
(* The key issue is that, there is more than one representation of zero
  Here is a expression as counter example
  Compute nat_to_bin(bin_to_nat (D' (D (D (D O))))).
  and the result is D' O  
*)
...
Fixpoint normalize (b : bin) : bin :=
  match b with
    | O => O
    | D b' => nat_to_bin (bin_to_nat (D b'))
    | D' b' => nat_to_bin (bin_to_nat (D' b'))
  end.

Theorem normalize_convert : forall b : bin, nat_to_bin(bin_to_nat(b)) = normalize(b).
Proof.
  intros n. induction n as [ | n' | n''].
  -simpl. reflexivity.
  -simpl. reflexivity.
...
(** Translate your solution for [plus_comm] into an informal proof:
    ...
    - First, suppose [n = 0].  We must show
        ...
        0 + m = m , This follows directly from the definition of [+].
        
        m + 0 = m , This follows the lemma plus_n_O
    ...
    from the induction hypothesis then,
    
        S (m + n') = m + (S n').
    
    which is immediately proved by the plus_n_Sm lemma _Qed_. *)
*)
(** [] *)
...
(** $Date: 2016-05-26 15:17:19 -0500 (Thu, 26 May 2016) $ *)
